...
- **函数返回对象的作用**
  - 每次组件实例化时，`data` 函数会**返回一个新的独立数据对象**：
    ```javascript
    // ✅ 正确：每个实例有自己的数据
    data() {
...
- **方法是无状态的**
  ...
  - 共享方法可以节省内存，无需每个实例重新创建一遍函数。
    ...
    methods: {
      increment() {
        this.count++; // 操作的是当前实例的 data
      }
    }
    ```

- **性能优化**
  - 如果是函数返回对象（类似 `data`），每次实例化都需要重新创建方法，浪费内存。
...
- **统一性**
  - Vue 的**所有生命周期钩子**（`created`、`updated`、`destroyed` 等）都是函数，保持设计一致性。
